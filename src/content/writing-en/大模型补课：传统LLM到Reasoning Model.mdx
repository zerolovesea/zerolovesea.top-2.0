...
>
> This led to System 2 Attention (S2A), a technique in which the model rewrites the input prompt into a cleaner form to improve accuracy.

![](file:///Users/zyaztec/Projects/Brevia/frontend/assets/brevia-logo-reveal.gif)
![](20260824-2.jpeg)
...
> A useful explanation, again from the [overview](https://zhuanlan.zhihu.com/p/1992701789498279934): LLM decoding continually selects the most probable sequence. For a given question, that sequence contains both a reasoning path and a final answer. The existence of the reasoning path means its final answer need not be the theoretically most likely one. Put differently, if the patterns and tokens in a reasoning path feel especially familiar and comfortable to the LLM, it may prefer that sequence even when the final answer is wrong.
...
That is o1's most important breakthrough: a model can explore reasoning paths for itself instead of being given those paths by humans.
...
For R1-Zero<Sidenote id="3">R1-Zero was built on the 671B-parameter DeepSeek-V3 base model, released in December 2024.</Sidenote>, researchers designed two kinds of reward. **Accuracy rewards** used the LeetCode compiler to verify code-problem answers and deterministic rules to evaluate math results. **Format rewards** used an LLM judge to check whether an answer followed the required structure—for example, placing its reasoning inside `<think>` tags.
...
R1-Zero was a major breakthrough, but its reasoning was difficult for people to read: languages were mixed, formatting was irregular, and the expression was hard to follow. DeepSeek-R1 therefore added SFT back on top of R1-Zero.
